...
function build_and_push_image () {
  ...
  docker buildx build -t balenablocks/bluetooth:$BALENA_MACHINE_NAME --platform $DOCKER_ARCH --file Dockerfile.$BALENA_MACHINE_NAME .
  
  echo "Publishing..."
  docker push balenablocks/bluetooth:$BALENA_MACHINE_NAME
